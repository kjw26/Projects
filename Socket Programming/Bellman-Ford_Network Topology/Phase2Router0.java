...
public class Phase2Router0 {
	...
	//The use of datagrams to complete this assignment was based upon the listings on oracle documentation. See references
	...
	public static void main(String[] args) throws UnknownHostException {
        ...
        String myIP="128.235.208.201";
        String IP1 = "128.235.208.225";
        String IP2 = "128.235.209.204";
        String IP3 = "128.235.209.205";
        ...
        try{
        	...
        	System.out.println("Sending packet to router3");
        	Send(socket, address3, 6561 , routingtable, myIP);
            	...
            	while(!Arrays.deepEquals(routingtable, goal)){
	    			...
	    			if(incmsg.getIdentifier().compareTo(IP3) == 0){
	    				System.out.println("recieved message from router3\n");
	    		        for(int[] list : arr){
	    		        	System.out.println(Arrays.toString(list));
	    		        }
	    				for(int j=0; j<4; j++){
	    					routingtable[3][j] = arr[3][j];
	    				}
	    			}
	...
	static int[][] BF(int[][] arr){
		...
		for(int i=0; i<4; i++){
			minarr[0] = arr1[0][0] + arr1[0][i];
			minarr[1] = arr1[0][1] + arr1[1][i];
			minarr[2] = arr1[0][2] + arr1[2][i];
			minarr[3] = arr1[0][3] + arr1[3][i];
